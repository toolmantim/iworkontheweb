...
#            li(:class => "where-it-all-started") { a "Where it all started", :href => R(Show, "1-lisa-herrod") }
  ...
  def add_your_profile
    div :class => "add-your-profile" do
      ...
      h3 "Not showing up?"
      p { %(There seems to be a bug with the flickr API search as some photos aren't showing up. Theyrsquo;ve acknowledged the bug so, fingers crossed, it should be resolved shortly.) }
    end
  end
